...
send.addEventListener("click", (event) => {
  ...
  // Variables pour stocker les résultats des validations
  ...
  // Vérifiez que toutes les validations sont correctes
...
function verifname(_nom) {
  if (_nom.length < 3) {
    ...
    onom.innerText = ""; // Efface le message en cas de succès
...
function veriffirstname(_prenom) {
  if (_prenom.length < 3) {
    ...
    oprenom.innerText = ""; // Efface le message en cas de succès
...
function validateddate(_date) {
  ...
  // Vérifie si le jour spécifié dépasse le nombre de jours maximum pour le mois donné
...
function verifconfidentialcode(_code) {
  ...
  if (code_valide.test(_code) == true) {
    ocode.innerText = ""; // Efface les messages d'erreur
    return true;
  } else {
